...
class MultipleEncoders(tf.keras.Model):
    ''' 1 Layer Bidirectional LSTM, 1 encoder for persona, 1 for input_msg '''
    def __init__(self, vocab_size, embedding_matrix, n_units, batch_size):
        super(MultipleEncoders, self).__init__()
    ...
    def call(self, inputs):
        '''
        inputs: [input_persona, input_utterence]
        returns: persona_encoder_states, utterence_encoder_states, [h1, c1]
        '''
    ...
    tf.saved_model.save(encoder, encoder_fn , signatures=encoder.call.get_concrete_function(
        [
            ...
            [
                ...
                tf.TensorSpec(shape=[None, LSTM_DIM], dtype=tf.float32, name='initial_h_bck'),
                tf.TensorSpec(shape=[None, LSTM_DIM], dtype=tf.float32, name='initial_c_bck')
            ]
        ]))
...
def beam_search_seq2seq(encoder_model, decoder_model, tokenizer, input_msg, in_seq_length, out_seq_length, beam_length = 3):
    ...
    encoder_init = [tf.zeros((1, LSTM_DIM))] * 4
    encoder_out, initial_state = encoder_model([input_seq, encoder_init])
